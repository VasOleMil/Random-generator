...
* **Dimension-Independent Mass Distribution (Center Reachable):** By scaling the element radius difference inversely to the space dimension ($dR = d/Rn$), the stepping function is regularized against the "curse of dimensionality." This constraints the mass ratio between the heaviest and lightest hard spheres to a strict upper bound: $M_{min}/{M_{max} \to \exp(d)} \approx 1.105,\quad d=0.1$. This prevents the system from generating massive directional mass imbalances, ensuring a stable, contrast-invariant mass distribution that does not polarize or distort as dimensions scale upward.
