...
_Note on control long-time memory tails:_  to identify precisely the termination of the thermalization phase (typically $400K+$ asynchronous steps for 100 elements), the system tracks $Ta$ — an Exponential Moving Average (EMA) calculated over an $Ar = 64$ step window. Furthermore, to prevent smooth, non-ergodic energy drift ($Ka = R_n \cdot kT$), a modified multiplier can be implemented within `EmtsCollBE`: the introduction of $Ds$ (near-LSB bit-doping) keeps the system parameters securely anchored within the corresponding physical range.

_Note on Boundaries:_ Boundary alignment using a static geometric size (`Pbound`) allows for clean analytical derivation. However, to align the boundary directly with a dynamic physical model—where the boundary-to-element interaction ratio is perfectly balanced—the full Compressor model configuration must be used, accounting for exact particle quantities and geometric radii.
